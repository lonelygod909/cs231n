...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        self.W2 = self.params['W2']
        self.b2 = self.params['b2']
        tmp = X.reshape(X.shape[0], -1).dot(self.W1) + self.b1
        scores = np.maximum(0, tmp).dot(self.W2) + self.b2
        ...
        dW2 += np.maximum(0, tmp).T.dot(dscores)
        dtmp = dscores.dot(self.W2.T)
        dtmp[tmp <= 0] = 0
        db1 += np.sum(dtmp, axis=0)
        dW1 += X.reshape(X.shape[0], -1).T.dot(dtmp)
