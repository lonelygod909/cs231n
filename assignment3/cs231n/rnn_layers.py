...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    N, T, _ = x.shape
    xs = np.split(x, T, axis=1)
    hs = [h0]
    caches = []
    for t in range(T):
        xt = xs[t].reshape(N, -1)
        ht = hs[t]
        next_h, cache = rnn_step_forward(xt, ht, Wx, Wh, b)
        hs.append(next_h)
        caches.append(cache)
...
def rnn_backward(dh, cache):
    ...
    dWh = np.zeros((H, H))
    db = np.zeros(H)
    dhs_atloss = np.split(dh, T, axis=1)
    for t in reversed(range(T)):
        dh_atloss = dhs_atloss[t].reshape(N, -1)
        for i in reversed(range(t + 1)):
            dxi, dprev_h, dWxi, dWhi, dbi = rnn_step_backward(dh_atloss, caches[i])
            dx[:, i, :] += dxi
            dWx += dWxi
            dWh += dWhi
            db += dbi
