...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        H = self.n_head
        query = query.view(N, S, H, E // H).permute(0, 2, 1, 3)
        key = key.view(N, T, H, E // H).permute(0, 2, 3, 1)
        value = value.view(N, T, H, E // H).permute(0, 2, 1, 3)
        weights = torch.matmul(query, key) / math.sqrt(E // H) # N, H, S, T
        if attn_mask is not None:
          weights = torch.masked_fill(weights, (attn_mask == 0).view(1, 1, S, T), float('-inf'))
        weights = self.attn_drop(torch.softmax((weights), dim=3))
        tmp = torch.matmul(weights, value) # N, H, S, E // H
        output = self.proj(tmp.permute(0, 2, 1, 3).reshape(N, S, E))
