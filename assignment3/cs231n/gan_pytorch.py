...
def discriminator_loss(logits_real, logits_fake):
    ...
    real_labels = torch.ones(logits_real.shape[0]).type(dtype)
    loss = bce_loss(logits_real, real_labels) + bce_loss(logits_fake, real_labels * 0)
...
def generator_loss(logits_fake):
    ...
    real_labels = torch.ones(logits_fake.shape[0]).type(dtype)
...
def build_dc_classifier(batch_size):
    ...
    # input (N, C, H, W) = (N, 1, 28, 28)
    model = nn.Sequential(
        nn.Conv2d(1, 32, 5, 1), # (N, 32, 24, 24)
        nn.LeakyReLU(),
        nn.MaxPool2d(2, 2), # (N, 32, 12, 12)
        nn.Conv2d(32, 64, 5, 1), # (N, 64, 8, 8)
        nn.LeakyReLU(),
