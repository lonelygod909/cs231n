...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        if (self.num_layers == 1):
            ...
            for i in range(self.num_layers-1):
                self.params[f"W{i+1}"] = np.random.normal(0, weight_scale, (input_dim if i == 0 else hidden_dims[i-1], hidden_dims[i]))
            ...
            self.params[f"W{self.num_layers}"] = np.random.normal(0, weight_scale, (hidden_dims[-1] if self.num_layers > 1 else input_dim, num_classes))
    ...
    def loss(self, X, y=None):
        ...
        if self.num_layers == 1:
            ...
            for i in range(self.num_layers):
                ...
                if i == self.num_layers - 1:
                    ...
                    if self.normalization == "batchnorm":
                        scores, cache = affine_bn_relu_forward(scores, W, b, self.params[f"gamma{i+1}"], self.params[f"beta{i+1}"], self.bn_params[i])
                    if self.normalization == "layernorm":
                        scores, cache = affine_ln_relu_forward(scores, W, b, self.params[f"gamma{i+1}"], self.params[f"beta{i+1}"], self.bn_params[i])
                    else:
        ...
        for i in reversed(range(self.num_layers)):
            if i == self.num_layers - 1:
                dx, dW, db = affine_backward(dscores, caches[i])
            else:
                if self.use_dropout:
