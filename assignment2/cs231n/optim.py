...
def rmsprop(w, dw, config=None):
    ...
    squared_grad = config["cache"]
    decay_rate = config["decay_rate"]
    learing_rate = config["learning_rate"]
    new_cache = decay_rate * squared_grad + (1 - decay_rate) * dw**2
    next_w = w - learing_rate * dw / (np.sqrt(new_cache) + config["epsilon"])
...
def adam(w, dw, config=None):
    ...
    v = config["v"] * config["beta2"] + (1 - config["beta2"]) * dw**2
    config["m"] = m
    config["v"] = v
    config["t"] += 1
